...
Axiom state_to_letter_In : forall ls s,
  StateSet.mem s (positions ls) = true ->
  exists cs, LetterSet.S.mem (cs , s) ls = true.
Definition split_finals_ad : forall r i first last next s,
    regex_match r s -> NFA_path next s first last ->
    StateSet.mem last (finals_ad r i first) = true ->
    (exists c, LetterSet.S.mem (c , last)
                 (d (annotate_helper i r).(fst)) = true) \/
    (l (annotate_helper i r).(fst) = true /\ first = last) :=
  fun r i first last next s =>
    match s with
    | EmptyString => fun memp pemp _ => or_intror (conj
      (match_emp_l r i EmptyString memp eq_refl)
      pemp)
    | String c s => fun _ _ last_final =>
      let H1
       : StateSet.mem last (positions (d (annotate_helper i r).(fst))) = true
       := Admit in
      or_introl (state_to_letter_In (d _) last H1)
    end.
...
Definition regex_match_to_path_result_weaken r1 r2 i1 i2 s next first
  ...
  := fun H1 H2 B => match B with
     ...
     | or_intror (ex_intro _ lt (conj lt_d path)) => or_intror
       (ex_intro _ lt (conj
        (LetterSet.S.mem_1 (H2 lt (LetterSet.S.mem_2 lt_d)))
        path))
     end.
...
Definition regex_match_to_path_result_compose :
  ...
  (forall lt, LetterSet.S.mem lt (d ar1) = true ->
  ...
  | or_intror (ex_intro _ lt (conj lt_d path1)) =>
    ...
    | or_introl (conj r2_emp s2_emp) => match s2_emp with eq_refl =>
      ...
      with eq_refl => or_intror (ex_intro _ lt (conj
        (LetterSet.S.mem_1 (H2 lt r2_emp (LetterSet.S.mem_2 lt_d)))
    ...
    | or_intror (ex_intro _ lt2 (conj lt2_d path2)) =>
      or_intror (ex_intro _ lt2 (conj
        (LetterSet.S.mem_1 (H3 lt2 (LetterSet.S.mem_2 lt2_d)))
        (NFA_path_compose _ _ _ _ _ _ path1 path2)))
    end
  end.

(* forward direction take 2 *)
Fixpoint regex_match_to_path (r : regex CharSet.t) (i : Z) (s : string)
  ...
  | mChar cs c c_in_cs => fun H1 H2 =>
    ...
    or_intror (ex_intro _ (cs , i) (conj
      (LetterSet.S.mem_1 A1)
      (ex_intro _ i (conj A2 eq_refl))))
      ...
      (fun elast elast_d =>
        regex_match_to_path f _ s2 next elast.(snd) mfs2
            (fun lt lt_in_pf => H2 elast lt (Letter2Set.S.union_3 _
             (LetterSet_prod_inc _ _ _ _ (LetterSet.S.mem_2 elast_d) lt_in_pf)))
            (fun lt1 lt2 A => H2 lt1 lt2
             (Letter2Set.S.union_2 _ (Letter2Set.S.union_3 _ A))))
      Be
...
Definition regex_match_to_path' r s (m : regex_match r s)
  ...
  := let B :=
     ...
     | or_intror (ex_intro _ lt (conj lt_d path)) =>
       ex_intro _ lt.(snd) (conj
         (StateSet.mem_1 (d_in_finals r 1 0 lt (LetterSet.S.mem_2 lt_d)))
         path)
     end.
